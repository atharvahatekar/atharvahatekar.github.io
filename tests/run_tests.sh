...
# grep -P refuses to run under a non-UTF-8 locale (common on Git Bash/Windows).
export LC_ALL="${LC_ALL:-C.UTF-8}"

# Every source file here is UTF-8. Without this, Python on Windows defaults to
# cp1252 and either crashes or silently hashes mis-decoded bytes.
export PYTHONUTF8=1

# Pick an interpreter that actually runs. On Windows, `python3` is often a
# Microsoft Store stub that resolves but exits without executing anything.
PY=""
...
if [ -z "$PY" ]; then
    echo "FAILED: no working Python interpreter found (need python3 or python)"
    exit 1
fi

# A native Python on Windows cannot open MSYS-style /d/... paths. `cygpath -m`
# yields D:/... (forward slashes, so it stays safe inside Python string literals).
REPO_ROOT_PY="$REPO_ROOT"
if command -v cygpath >/dev/null 2>&1; then
    REPO_ROOT_PY="$(cygpath -m "$REPO_ROOT")"
fi
...
HTML_FILES=("index.html" "cv.html" "projects.html")
...
STYLE_PY="$REPO_ROOT_PY/style.css"
...
if "$PY" -c "import xml.etree.ElementTree as ET; ET.parse('$REPO_ROOT_PY/sitemap.xml')" 2>/dev/null; then
...
s = open('$REPO_ROOT_PY/index.html').read()
