    ...
    function buildCommands() {
        return [
            ...
            { group: 'RUN', icon: '$', label: 'download_resume --pdf', hint: 'file', run: () => {
                ...
                a.href = 'content/Atharva_Hatekar_CV_2026.pdf';
                a.download = 'Atharva_Hatekar_CV_2026.pdf';
                document.body.appendChild(a);
                a.click();
                a.remove();
            } },
            { group: 'RUN', icon: '$', label: 'lang --set en', hint: 'English', run: () => window.i18n && window.i18n.setLang('en') },
            { group: 'RUN', icon: '$', label: 'lang --set de', hint: 'Deutsch', run: () => window.i18n && window.i18n.setLang('de') },
            { group: 'CONNECT', icon: '@', label: 'github.com/atharvahatekar', hint: '↗', run: () => window.open('https://github.com/atharvahatekar', '_blank', 'noopener') },
            { group: 'CONNECT', icon: '@', label: 'linkedin.com/in/atharvahatekar', hint: '↗', run: () => window.open('https://linkedin.com/in/atharvahatekar', '_blank', 'noopener') },
            { group: 'CONNECT', icon: '@', label: 'mail atharva.hatekar@outlook.com', hint: '✉', run: () => { window.location.href = 'mailto:atharva.hatekar@outlook.com'; } },
        ];
    }
